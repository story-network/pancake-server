...
public class PancakeLauncher {
    ...
    public static PancakeLauncher createLauncher(URL serverPath, String[] args) {
        if (launcher != null)
            throw new RuntimeException("Launcher instance is already made");

        return launcher = new PancakeLauncher(serverPath, args);
    }
    ...
    private PancakeLauncher(URL serverPath, String[] args) {
        this.args = args;
    ...
    protected String provideServerMapping(String version, boolean recache) {
        ...
        if (rawServerMapping == null || !info.downloads.serverMappings.sha1
                .equalsIgnoreCase(Hex.byteArrayToHex(Hash.sha1From(rawServerMapping)))) {
    ...
    public void launch() {
        ...
        this.serverClassLoader = new ServerClassLoader(launcherClassLoader, mixinModder);

        serverClassLoader.addURL(serverPath);
        ...
        if (serverOptional.isPresent()) {
            ...
            LOGGER.fatal("Cannot find IPancakeServer service. Server cannot start");
            return;
        }
